...
func SQLExport() error {
	...
	sqlPath := fmt.Sprintf("msql_%s.sql", timer)
	//docker
	//cmd := exec.Command("docker", "exec", "mysql", "mysqldump", "-u"+mysql.Username, "-p"+mysql.Password, mysql.DBName)
	//mysql本地
	cmd := exec.Command("C:\\Program Files\\MySQL\\MySQL Server 8.0\\bin\\mysqldump.exe", "-u"+mysql.Username, "-p"+mysql.Password, mysql.DBName)
