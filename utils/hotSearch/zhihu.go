...
func (*Zhihu) GetHotSearchData(maxNum int) (HotSearchData other.HotSearchData, err error) {
	...
	for i := 0; i < maxNum; i++ {
		...
		hotList = append(hotList, other.HotItem{
			...
			Image:       fmt.Sprintf(gjson.Get(jsonStr, "initialState.topstory.hotList."+strconv.Itoa(i)+".target.imageArea.text").Str),
			Popularity:  gjson.Get(jsonStr, "initialState.topstory.hotList."+strconv.Itoa(i)+".target.metricsArea.text").Str,
			URL:         fmt.Sprintf(gjson.Get(jsonStr, "initialState.topstory.hotList."+strconv.Itoa(i)+".target.link.text").Str),
		})
