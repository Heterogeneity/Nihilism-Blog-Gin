...
func (articleService *ArticleService) ArticleCreate(req request.ArticleCreate) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		illustration, err := utils.FindIllustrations(articleToCreate.Content)
		if err != nil {
			return err
		}
		if err := utils.ChangeImagesCategory(tx, illustration, appTypes.Illustration); err != nil {
			return err
		}
...
func (articleService *ArticleService) ArticleDelete(req request.ArticleDelete) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		for _, id := range req.IDs {
			...
			illustration, err := utils.FindIllustrations(articleToDelete.Content)
			if err != nil {
				return err
			}
			if err := utils.InitImagesCategory(tx, illustration); err != nil {
				return err
			}
			// 同时删除该文章下的所有评论
...
func (articleService *ArticleService) ArticleUpdate(req request.ArticleUpdate) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		oldIllustration, err := utils.FindIllustrations(oldArticle.Content)
		if err != nil {
			return err
		}
		...
		addedIllustrations, removedIllustrations := utils.DiffArrays(oldIllustration, newIllustrations)
