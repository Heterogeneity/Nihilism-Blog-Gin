...
func (feedbackService *FeedbackService) FeedbackInfo(uid uuid.UUID) (feedbacks []database.Feedback, err error) {
	err = global.DB.Model(&database.Feedback{}).Order("id desc").Where("user_uid = ?", uid).Find(&feedbacks).Error
	if err != nil {
		return nil, err
	}
